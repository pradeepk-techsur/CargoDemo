...
# CargoDemo is a Fastify API (src/server/index.ts) that binds the host in
# CARGODEMO_HOST and serves the built React SPA (dist/client) same-origin on
# CARGODEMO_PORT. The config warns and the preview proxy cannot reach a
# loopback bind, so force 0.0.0.0 here.
...
#
# Exec target (D-18): the project's OWN single-command contract, `npm start`.
# Its `prestart` (scripts/ensureClientBuild.mjs) builds dist/client — which is
# gitignored and therefore ABSENT on a fresh clone (runtime-environment.md §7),
# so the SPA the Fastify server serves same-origin must be built here, not
# assumed. `start` then runs src/server/index.ts, which migrates the SQLite
# schema, seeds-if-empty, and listens on 0.0.0.0:3000. `npm run dev`
# (tsx watch) is deliberately NOT used: it skips `prestart`, so it would serve
# a "client bundle not built" notice instead of the app. Migrate→seed→serve is
# in-process (native SQLite app, no compose), using the project's own tooling.
EXEC_CMD='npm start'
